...
const PDFGenerator = ({ 
    ...
    reportElement.innerHTML = `
      ...
      <div style="border-top: 2px solid #333; padding-top: 20px; margin-top: 30px;">
        ...
        <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
          <p style="color: #9ca3af; font-size: 12px; font-style: italic;">Generated by Party Budget Planner</p>
        </div>
      </div>
    `;
  ...
  const generatePDFReport = () => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    //Alcoholic people info
    doc.text(`• Alcoholic people: ${alcoholicPeople} (Cost per person: Rs.${alcoholicPeople > 0 ? alcoholicCostPerPerson.toLocaleString() : '0'})`, 20, yPosition);
    yPosition += 10;
    //Non-alcoholic people info
    doc.text(`• Non-alcoholic people: ${nonAlcoholicPeople} (Cost per person: Rs.${nonAlcoholicPeople > 0 ? nonAlcoholicCostPerPerson.toLocaleString() : '0'})`, 20, yPosition);
    const nonAlcoholicText = `• Non-alcoholic people: ${nonAlcoholicPeople} (Cost per person: Rs.${nonAlcoholicPeople > 0 ? nonAlcoholicCostPerPerson.toLocaleString() : '0'}`;
    const nonAlcoholicLines = doc.splitTextToSize(nonAlcoholicText, 85);
    
    // Move to bottom for generated by text
    yPosition += Math.max(8, nonAlcoholicLines.length * 10) + 20;

    // Add generated by text at bottom center
    doc.setFontSize(10);
    doc.setTextColor(150, 150, 150);
    const generatedByText = 'Generated by Party Budget Planner';
    const textWidth = doc.getTextWidth(generatedByText);
    const centerX = (doc.internal.pageSize.getWidth() - textWidth) / 2;
    doc.text(generatedByText, centerX, yPosition);
